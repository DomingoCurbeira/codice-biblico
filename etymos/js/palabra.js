document.addEventListener('DOMContentLoaded', () => {
    ...
    // 1. Obtener el ID de la URL (ej: palabra.html?id=hesed-heb)
    ...
    async function cargarPalabra() {
        try {
            const respuesta = await fetch('../data/etymos/lexico.json'); // Asegúrate que la ruta sea correcta
    ...
    function renderizarDetalle(palabra) {
        ...
        // ==============================================
        // --- ACTUALIZAR METADATOS Y TÍTULO DINÁMICAMENTE ---
        // ==============================================
        ...
        if (ogTitle) ogTitle.content = `${palabra.original} - ${palabra.palabra_espanol}`;
        if (ogDesc) ogDesc.content = palabra.definicion_corta;
        // ==============================================
        ...
        contenedorDetalle.innerHTML = `
            ...
            <div class="etymos-share-section">
                ...
                <div class="share-actions">
                    ...
                    <button id="btn-nota-lexico" class="btn-share-round nota" aria-label="Crear Nota">
    ...
    // ==============================================
    // --- LÓGICA DE BOTONES DE ACCIÓN (NUEVA) ---
    // ==============================================

    // Función para manejar la creación de notas (Redirige a Escriba)
    function handleCrearNota() {
        // Extraemos los textos de la pantalla actual
        ...
        // Tomamos el párrafo de la definición corta
        const definicion = document.querySelectorAll('.detalle-seccion p')[1].innerText; 

        // Armamos el texto para Escriba
        const tituloNota = `Léxico: ${palabraOriginal} (${palabraEspanol})`;
        const cuerpoNota = `Estuve profundizando en Etymos sobre la palabra original "${palabraOriginal}" que significa "${palabraEspanol}".\n\nDefinición:\n${definicion}\n\nMis reflexiones:\n`;
        
        // Creamos la URL apuntando a tu app de notas y redirigimos
        const url = `../notas/index.html?titulo=${encodeURIComponent(tituloNota)}&cuerpo=${encodeURIComponent(cuerpoNota)}`;
    ...
    function handleCopiarEnlace(boton) {
        navigator.clipboard.writeText(window.location.href).then(() => {
            // Añadir clase para la animación verde
            boton.classList.add('copied');
            
            // Quitarla después de 2 segundos para que pueda volver a usarse
            setTimeout(() => {
                boton.classList.remove('copied');
            }, 2000);

            // Toast de éxito (Si tienes un sistema de toast global)
    ...
    // EVENTOS: Añadimos los escuchadores a los nuevos botones
    document.addEventListener('click', (e) => {
        // Detectar click en el botón de Nota
        if (e.target.closest('#btn-nota-lexico')) {
            handleCrearNota();
        }
        
        // Detectar click en el botón de Copiar Enlace
        const btnCopy = e.target.closest('#btn-copy-link');
        if (btnCopy) {
            handleCopiarEnlace(btnCopy);
        }
    });
