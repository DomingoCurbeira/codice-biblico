...
const huellasPath = 'proyectos/biblia/codice/data/estudios/huellas.json';
let huellasData = JSON.parse(fs.readFileSync(huellasPath, 'utf8'));
huellasData = huellasData.filter(e => e.id !== mon_study.id);
huellasData.unshift(mon_study);
fs.writeFileSync(huellasPath, JSON.stringify(huellasData, null, 2));

const mitosPath = 'proyectos/biblia/codice/data/estudios/mitos.json';
let mitosData = JSON.parse(fs.readFileSync(mitosPath, 'utf8'));
mitosData = mitosData.filter(e => e.id !== tue_study.id);
mitosData.unshift(tue_study);
fs.writeFileSync(mitosPath, JSON.stringify(mitosData, null, 2));

console.log('✅ Estudios Lunes y Martes (Semana 6) inyectados exitosamente.');
